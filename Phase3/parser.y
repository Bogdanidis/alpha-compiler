...
expr    : assignexpr                {$$=$1;assignflag=1;printf("expr-->assignexpr\n");}
        ...
        | expr OR{
                      if(!$1->isBoolean&&$1->isEmitable){
                          if(!$1->passedByNot){
                            ...
                            emit(if_eq,newexpr_constbool(1),$1,NULL,-1,nextquadlabel());
                            emit(jump,NULL,NULL,NULL,-1,nextquadlabel());
                            $1->isEmitable=0;
                          }else
                                $1->passedByNot=0;
                                    ...
                                    backpatch($1->falselist,$4);
                                      ...
                                      if(!$5->isBoolean){
                                          if(!$5->passedByNot){
                                            ...
                                            emit(if_eq,newexpr_constbool(1),$5,NULL,-1,nextquadlabel());
                                            emit(jump,NULL,NULL,NULL,-1,nextquadlabel());
                                            $5->isEmitable=0;
                                          }else
                                                $5->passedByNot=0;
        ...
        | NOT expr              {
                                    ...
                                    emit(if_eq,newexpr_constbool(1),$2,NULL,-1,nextquadlabel());
                                    emit(jump,NULL,NULL,NULL,-1,nextquadlabel());
        ...
        | expr MODULO expr		          {
                                      ...
                                      printf("expr-->expr % expr\n");}
        | expr EQUAL {    if($1->backpatch&&!assignflag){
              expr* t=newexpr(boolexpr_e),*t1=newexpr(boolexpr_e),*arg=newexpr(var_e);
              t->type=constbool_e;
              t->boolConst=1;
              arg->type=var_e;
              ...
              assignflag=0;
                                    ...
                                    if($1->passedByNot){
                                      ...
                                      strcpy($1->strConst,arg->sym->name);
                                    ...
                                    if($4->passedByNot){
                                      ...
                                      strcpy($4->strConst,arg->sym->name);
                                    ...
                                    emit(if_eq,$1,$4,NULL,0,currQuad);

                                    emit(jump,NULL,NULL,NULL,-1,currQuad);

                                    printf("expr-->expr == expr\n");}
        | expr NOT_EQUAL {    if($1->backpatch&&!assignflag){
                                expr* t=newexpr(boolexpr_e),*t1=newexpr(boolexpr_e),*arg=newexpr(var_e);
                                t->type=constbool_e;
                                t->boolConst=1;
                                arg->type=var_e;
 ...
 expr	      {
                                    ...
                                    $$->truelist=makelist(nextquadlabel(),$$->truelist);
                                    $$->falselist=makelist(nextquadlabel()+1,$$->falselist);
                                    emit(if_noteq,$1,$4,NULL,0,currQuad);
                                    emit(jump,NULL,NULL,NULL,-1,currQuad);
                                    printf("expr-->expr != expr\n");}
        ...
        | expr GREATER  expr	        {
                                    ...
                                    $$->truelist=makelist(nextquadlabel(),$$->truelist);
                                    $$->falselist=makelist(nextquadlabel()+1,$$->falselist);
                                    emit(if_greater,$1,$3,NULL,0,currQuad);
                                    emit(jump,NULL,NULL,NULL,-1,currQuad);
                                    printf("expr-->expr > expr\n");}
        |  expr  LESS  expr	       {
                                    $$->isBoolean=1;
                                    ...
                                    $$->truelist=makelist(nextquadlabel(),$$->truelist);
                                    $$->falselist=makelist(nextquadlabel()+1,$$->falselist);
                                    emit(if_less,$1,$3,NULL,-1,currQuad);
                                    emit(jump,NULL,NULL,NULL,-1,currQuad);
        ...
        | expr GREATER_EQUAL expr 	{
                                    ...
                                    $$->truelist=makelist(nextquadlabel(),$$->truelist);
                                    $$->falselist=makelist(nextquadlabel()+1,$$->falselist);
                                    emit(if_geatereq,$1,$3,NULL,-1,currQuad);
                                    emit(jump,NULL,NULL,NULL,-1,currQuad);
                                    printf("expr-->expr >= expr\n");}
        ...
        | expr LESS_EQUAL expr		  {
                                    ...
                                    $$->truelist=makelist(nextquadlabel(),$$->truelist);
                                    $$->falselist=makelist(nextquadlabel()+1,$$->falselist);
                                    emit(if_lesseq,$1,$3,NULL,-1,currQuad);
                                    emit(jump,NULL,NULL,NULL,-1,currQuad);
                                    printf("expr-->expr <= expr\n");}
...
assignexpr  : lvalue{
              ...
              ASSIGNMENT expr   {
                                      ...
                                      if($1->type==tableitem_e){
                                          emit(tablesetelem,$1->index,$4,$1,-1,nextquadlabel());
                                          $$=emit_iftableitem($1);
                                          $$->type=assignexpr_e;
                                      }else{
                                      if($4->backpatch){
                                          ...
                                          $$=$4;
                                            ...
                                            emit(assign,tmp_expr,NULL,$1,-1,nextquadlabel());
                                            $$=newexpr(assignexpr_e);
                                            $$->sym=newtemp();
                                            $$->strConst=malloc(sizeof(strlen($$->sym->name))+1);
                                            strcpy($$->strConst,$$->sym->name);
                                            $$->type=var_e;
...
term    : LEFT_PARENTHESIS expr RIGHT_PARENTHESIS {$$=$2;printf("term-->(expr)\n");}
          ...
          | INCREMENT lvalue                        {
               ...
               if(check_arithmetic($2)){
                    if($2->type==tableitem_e){
                      ...
                      emit(tablesetelem,$2->index,$$,$2,-1,nextquadlabel());
                    }else{
                      emit(add,newexpr_constint(1),$2,$2,-1,nextquadlabel());
                      $$=newexpr(arithexpr_e);
                      $$->sym=newtemp();
          ...
          | lvalue INCREMENT                        {
                ...
                if($1->type==tableitem_e){
                    ...
                    emit(assign,val,NULL,$$,-1,nextquadlabel());
                    emit(add,val,newexpr_constint(1),val,-1,nextquadlabel());
                    emit(tablesetelem,$1->index,val,$1,-1,nextquadlabel());
                }else{
                    emit(assign,$1,NULL,$$,-1,nextquadlabel());
                    emit(add,$1,newexpr_constint(1),$1,-1,nextquadlabel());
                }
                }
          ...
          | DECREMENT lvalue                        {
                  ...
                  if(check_arithmetic($2)){
                      if($2->type==tableitem_e){
                        ...
                        emit(sub,$2,newexpr_constint(1),$2,-1,nextquadlabel());
                        $$=newexpr(arithexpr_e);
                        $$->sym=newtemp();
                        emit(assign,$2,NULL,$$,-1,nextquadlabel());
                      }
          ...
          | lvalue DECREMENT                        {
                  ...
                  if($1->type==tableitem_e){
                      ...
                      emit(assign,$1,NULL,$$,-1,nextquadlabel());
                      emit(sub,$1,newexpr_constint(1),$1,-1,nextquadlabel());
                  }
                  }
...
member       : lvalue DOT ID {
                    $$=member_item($1,yylval.stringValue);//ID.name
            ...
            | call LEFT_SQUARE_BRACKET expr RIGHT_SQUARE_BRACKET {$$=$1;arrayFlag = 1;printf("member--> call [ expr ]\n");$$=malloc(sizeof(expr));}
            ;
...
objectdef	: LEFT_SQUARE_BRACKET elist RIGHT_SQUARE_BRACKET	{
                ...
                int i=0;
                while($2){
                    emit(tablesetelem,newexpr_constint(i++),$2,t,-1,nextquadlabel());
                    $2=$2->next;
                }
                $$=t;
                ...
                emit(tablecreate,NULL,NULL,t,-1,nextquadlabel());
                while($2){
                    emit(tablesetelem,$2->index,$2->value,t,-1,nextquadlabel());
                    $2=$2->next;
                }
                $$=t;
...
block			: LEFT_CURLY_BRACKET RIGHT_CURLY_BRACKET	{$$=malloc(sizeof(expr));printf("block-->{}\n");}
			 	  | LEFT_CURLY_BRACKET {scope++;}  stmt statements RIGHT_CURLY_BRACKET{
            //    printf("$$ %p\n",$$);
            //    printf("$3 %p\n",$3);
            //    printf("$4 %p\n",$4);
                $$=malloc(sizeof(expr));
...
ifprefix	: IF LEFT_PARENTHESIS expr{
        ...
        emit(if_eq,$3,newexpr_constbool(1),NULL,nextquadlabel()+2,currQuad);
        $$=nextquadlabel();
...
whilecond :LEFT_PARENTHESIS expr{if($2->backpatch){
            ...
            emit(if_eq,$2,newexpr_constbool(1),NULL,nextquadlabel()+2,nextquadlabel());
            $$=nextquadlabel();
            emit(jump,NULL,NULL,NULL,0,nextquadlabel());
...
forprefix: FOR LEFT_PARENTHESIS elist SEMICOLON M expr{if($6->backpatch){
                            ...
                            //  printf("here\n");
                                      ...
                                      emit(if_eq,$6,newexpr_constbool(1),NULL,0,nextquadlabel());
...
N : {$$=nextquadlabel();emit(jump,NULL,NULL,NULL,0,nextquadlabel());}
  ;
  ...
  returnstmt	: RETURN{if (scope==0)printf("Error use of return in scope 0 in line %d\n",yylineno);}
   expr {if($3->backpatch){
                        ...
                        if(currscopespace()==functionlocal){
                                                emit(ret,$3,NULL,NULL,-1,nextquadlabel());
