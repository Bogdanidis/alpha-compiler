...
char* newtempvar(){
	assert(tempcounter<15);
	char *newName=malloc(sizeof(char)*32);
	strcpy(newName,"_t");
	char str[29];
	int i=0;
	for(i=0;i<29;i++)
...
expr* make_call(expr* lv,expr* elist){
	...
	while (temp){
			emit(param,temp,NULL,NULL,-1,nextquadlabel());
			temp=temp->next;
	}
	emit(call,func,NULL,NULL,-1,nextquadlabel());
	expr* result=newexpr(var_e);
	result->sym=newtemp();
	emit(getretval,NULL,NULL,result,-1,nextquadlabel());
	return result;

}


list* makelist(unsigned int quad,list* head){
